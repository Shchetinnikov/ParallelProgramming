...
__global__ void vector_add(const float* a, const float* b, float* c, int num_elements) {
	int i = blockDim.x * blocIdx.x + threadIdx.x;
...
int main() {
	size_t const num_elements = 50000000;
	...
	vector_add <<< num_blocks, block_size >>> (dev_a, deb_b, dev_c, num_elements);
